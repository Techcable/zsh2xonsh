...
@click.option("cmd", "--cmd", "-c", help="Translate a single input command")
@click.option(
    "assume_runtime",
    "--assume-runtime/--no-assume-runtime",
    "-r",
    is_flag=True,
    help="Assume the runtime is already present (instead of assuming it's already present)",
)
@click.option("stdin", "--stdin", is_flag=True, help="Read input from stdin")
@click.option(
    "assume_context",
    "--assume-context",
    "-c",
...
def zsh2xonsh(
    ...
    cmd=None,
    validate=False,
    assume_runtime=False,
    assume_context=False,
    stdin=False,
):
    ...
    if cmd is not None:
        ...
        raise click.ClickException(
            "Must specifiy either `--cmd` `--stdin` or an input file"
        )
    try:
